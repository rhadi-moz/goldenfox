...
G_EVAL_PROMPTS = "evaluator/g_eval_prompts.json"
MEASURE_RETRIES = 3
...
class EvaluationMetrics:
    def __init__(self, metrics):
        self._supported_metrics = {
            ...
            "prompt_alignment": {
                "method": self._prompt_alignment,
                "requires": [EvaluationFields.EXAMPLE],
            },
        }
        print(metrics)
        print(self._supported_metrics.keys())
    ...
    def _meteor(self, pred: list, ref: list):
        ...
        for p, r in zip(pred, ref):
            evals["meteor"].append(
                ev.compute(predictions=[p], references=[r])["meteor"]
            )
    ...
    def _g_eval_measure_with_retry(
        self, metric, test_case, max_retries=MEASURE_RETRIES
    ):
    ...
    def _g_eval(
        self, examples: list, pred: list, ref: list, task: str, use_ref: bool = True
    ) -> dict:
        ...
        try:
            for metric_name in prompt_templates[task]:
                ...
                for p, r, e in zip(pred, ref, examples):
                    ...
                    try:
                        result = self._g_eval_measure_with_retry(
                            metric, test_case, max_retries=MEASURE_RETRIES
                        )
                        evals_for_metric.append(result)
                    except ValueError as e:
                        # Handle the failure case
                        logger.error(
                            f"G-Eval measurement failed after {MEASURE_RETRIES} attempts: {e}"
                        )
                        raise e

                evals[metric_name] = evals_for_metric
                evals[f"{metric_name}_mean"] = np.mean(
                    [x["score"] for x in evals_for_metric]
                )
    ...
    def _prompt_alignment(self, examples: list, predictions: list, expected_output: list, instructions: list):


        evals = {}
        evals_for_metric = []
        for p, r, e, i in zip(predictions, expected_output, examples, instructions):
            logger.info("Running prompt alignment evaluation")
    ...
    def run_all(self, examples: list, pred: list, ref: list, instructions: list) -> EvalJobMetrics:
        print("chosen_metrics",self._chosen_metrics)
        ...
        for metric in self._chosen_metrics:
            ...
            if metric == 'prompt_alignment':
                results[metric] = self._supported_metrics[metric]["method"](
                    examples, pred, ref, instructions
                )
                print(results)

            elif EvaluationFields.EXAMPLE in self._supported_metrics[metric]["requires"]:
                results[metric] = self._supported_metrics[metric]["method"](
                    examples, pred, ref
                )
                ...
                results[metric] = self._supported_metrics[metric]["method"](examples,ref)
